    ...
    I2C: Read + Write + WriteRead
{
    ...
    pub fn restart(&mut self) -> Result<(), Error> {
        ...
        // Datasheet p.15
        let mut buffer = [0; 1];
        self.i2c_handle.write_read(self.address, &[MODE1], &mut buffer).map_err(|_| Error::ReadError)?;

        // Verify that RESTART bit is set
        if buffer[0] & 0x10 == 0x10 {
            // Bit Mask to clear bit 4, SLEEP bit
            self.i2c_handle.write(self.address, &[MODE1, buffer[0] & 0xEF]).map_err(|_| Error::WriteError)?; 
            // Wait 500us for oscillator to stabilize
            sleep(Duration::from_micros(500));
            // Write logic 1 to bit 7, RESTART bit
            self.i2c_handle.write(self.address, &[MODE1, buffer[0] | 0x10]).map_err(|_| Error::WriteError)?;
        }
        Ok(())
    }
    ...
    pub fn set_pwm_freq(&mut self, frequency: u32) -> Result<(), Error> {
        ...
        self.i2c_handle.write(self.address, &[MODE1, mode & 0x10]).map_err(|_| Error::WriteError)?;
        println!("{:b}", mode | 0x10);
        
        let prescale_value = (25_000_000 / (4096 * frequency)) as u8; // TODO: Add -1 back
        self.i2c_handle.write(self.address, &[PRE_SCALE, prescale_value]).map_err(|_| Error::WriteError)?; // TODO: Handle result
        
        let mut buffer2: [u8;1] = [0;1];
        self.i2c_handle.write_read(self.address, &[MODE1], &mut buffer2).map_err(|_| Error::ReadError)?;

        let mode2 = buffer2[0];
        println!("{mode2:b}");
        
        self.i2c_handle.write(self.address, &[MODE1, mode]).map_err(|_| Error::WriteError)?;
        Ok(())
    }
